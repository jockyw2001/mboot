...
extern MS_U32 u32GOPDbgLevel_u; // 0: slient, 1~5: small value for fewer log
// Debug Logs, level form low(INFO) to high(FATAL, always show)
// Function information, ex function entry
#define GOP_INFO(x, args...) if(u32GOPDbgLevel_u >= 5) { ULOGI("GOP API", x, ##args); }
// Warning, illegal paramter but can be self fixed in functions
#define GOP_WARN(x, args...) if(u32GOPDbgLevel_u >= 4) { ULOGW("GOP API", x, ##args); }
//  Need debug, illegal paramter.
#define GOP_DBUG(x, args...) if(u32GOPDbgLevel_u >= 3) { ULOGD("GOP API", x, ##args); }
// Error, function will be terminated but system not crash
#define GOP_ERR(x, args...) if(u32GOPDbgLevel_u >= 2) { ULOGE("GOP API", x, ##args); }
// Critical, system crash. (ex. assert)
#define GOP_FATAL(x, args...) if(u32GOPDbgLevel_u >= 1) { ULOGF("GOP API", x, ##args); }

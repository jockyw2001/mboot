...
typedef enum
{
    ...
    E_MVOP_CMD_SET_XC_GEN_TIMING,         ////<0x117 for dynamic enable timng triger from xc (including vsync and start).
    E_MVOP_CMD_SET_SEL_OP_FIELD,          ////<0x118  Set mvop output filed (new for sub).
    E_MVOP_CMD_SET_SIZE_FROM_MVD,         ////<0x119  Set mvop output h/w size from vdec (new for sub).
    E_MVOP_CMD_SET_CROP_START_POS,        ////<0x11A Set mvop crop start x/y (new for sub).
    E_MVOP_CMD_SET_IMAGE_WIDTH_HEIGHT,    ////<0x11B Set mvop crop  x/y size (new for sub).
    E_MVOP_CMD_SET_INV_OP_VS,             ////<0x11C Set mvop vsync inverse (new for sub).
    E_MVOP_CMD_SET_FORCE_TOP,             ////<0x11D Set mvop OP only top field (new for sub).
    ...
    E_MVOP_CMD_GET_CROP_FOR_XC,                 ///< 0x411 Set crop and Get crop infomation. Without set register.
    E_MVOP_CMD_GET_IS_XC_GEN_TIMING,            ///< 0x412Get if timing is from xc (or from mvop).
} MVOP_Command;

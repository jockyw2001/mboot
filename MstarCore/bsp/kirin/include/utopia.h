...
typedef void (*FUtopiaReg)(FUtopiaOpen ModuleType);

MS_U32 UtopiaRegisterToCore(FUtopiaReg pReg, FUtopiaOpen pOpen, char *modulename);

#define UTOPIA_OPEN_FUNC(x) MODULE_TYPE_##x##_FULL
#define UTOPIA_OPEN_FUNC_LITE(x) MODULE_TYPE_##x##_LITE
#define UTOPIA_REG_FUNC(x) x##RegisterToUtopia

#define UTOPIA_MODULE_INIT(ModuleName) \
static __attribute__((constructor)) void Module_Init##ModuleName(void) \
{   extern void UTOPIA_REG_FUNC(ModuleName) (FUtopiaOpen ModuleType);\
    printf("module %s register to core\n", #ModuleName);\
    UtopiaRegisterToCore((FUtopiaReg)UTOPIA_REG_FUNC(ModuleName),\
                        (FUtopiaOpen)UTOPIA_OPEN_FUNC(ModuleName),\
                        #ModuleName);\
}
#define UTOPIA_MODULE_INIT_LITE(ModuleName) \
static __attribute__((constructor)) void Module_Init##ModuleName(void) \
{   extern void UTOPIA_REG_FUNC(ModuleName) (FUtopiaOpen ModuleType);\
    printf("module %s register to core\n", #ModuleName);\
    UtopiaRegisterToCore((FUtopiaReg)UTOPIA_REG_FUNC(ModuleName),\
                        (FUtopiaOpen)UTOPIA_OPEN_FUNC_LITE(ModuleName),\
                        #ModuleName);\
}
#define UTOPIA_MODULE_INIT2(ModuleName) \
static __attribute__((constructor)) void Module_Init##ModuleName(void) \
{   extern void UTOPIA_REG_FUNC(ModuleName) (void);\
    UtopiaRegisterToCore((FUtopiaReg)UTOPIA_REG_FUNC(ModuleName),\
                        (FUtopiaOpen)UTOPIA_OPEN_FUNC(ModuleName),\
                        #ModuleName);\
}

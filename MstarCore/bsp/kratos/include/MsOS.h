...
// Init
//
DLL_PUBLIC MS_BOOL MsOS_Init (void);


//
// Memory management
//
DLL_PUBLIC MS_S32 MsOS_CreateMemoryPool (MS_U32 u32PoolSize,
                              MS_U32 u32MinAllocation,
                              void * pPoolAddr,
                              MsOSAttribute eAttribute,
                              char *pPoolName);

DLL_PUBLIC MS_BOOL MsOS_DeleteMemoryPool (MS_S32 s32PoolId);

DLL_PUBLIC MS_BOOL MsOS_InfoMemoryPool (MS_S32 s32PoolId,
                             void **pPoolAddr,
                             MS_U32 *pu32PoolSize,
                             MS_U32 *pu32FreeSize,
                             MS_U32 *pu32LargestFreeBlockSize);

DLL_PUBLIC void * MsOS_AllocateMemory (MS_U32 u32Size, MS_S32 s32PoolId);

//void * MsOS_AllocateAlignedMemory (MS_U32 u32Size, MS_U32 u32AlignedByte, MS_S32 s32PoolId);

DLL_PUBLIC void * MsOS_ReallocateMemory (void *pOrgAddress, MS_U32 u32NewSize, MS_S32 s32PoolId);

DLL_PUBLIC MS_BOOL MsOS_FreeMemory (void *pAddress, MS_S32 s32PoolId);

DLL_PUBLIC MS_S32 MsOS_CreateFixSizeMemoryPool (MS_U32 u32PoolSize,
                                     MS_U32 u32BlockSize,
                                     void * pPoolAddr,
                                     MsOSAttribute eAttribute,
                                     char *pPoolName);

DLL_PUBLIC MS_BOOL MsOS_DeleteFixSizeMemoryPool (MS_S32 s32PoolId);

DLL_PUBLIC MS_BOOL MsOS_InfoFixSizeMemoryPool (MS_S32 s32PoolId,
                                    void **pPoolAddr,
                                    MS_U32 *pu32PoolSize,
                                    MS_U32 *pu32FreeSize,
                                    MS_U32 *pu32LargestFreeBlockSize);

DLL_PUBLIC void * MsOS_AllocateFixSizeMemory (MS_S32 s32PoolId);

DLL_PUBLIC MS_BOOL MsOS_FreeFixSizeMemory (void *pAddress, MS_S32 s32PoolId);


//
// Task
//
DLL_PUBLIC MS_S32 MsOS_CreateTask (TaskEntry pTaskEntry,
...
DLL_PUBLIC MS_BOOL MsOS_DeleteTask (MS_S32 s32TaskId);

DLL_PUBLIC void MsOS_YieldTask (void);

DLL_PUBLIC void MsOS_DelayTask (MS_U32 u32Ms);

DLL_PUBLIC void MsOS_DelayTaskUs (MS_U32 u32Us);

DLL_PUBLIC void MsOS_DelayTaskUs_Poll(MS_U32 u32Us);

DLL_PUBLIC MS_BOOL MsOS_ResumeTask (MS_S32 s32TaskId);

DLL_PUBLIC MS_BOOL MsOS_SuspendTask (MS_S32 s32TaskId);

DLL_PUBLIC MS_S32 MsOS_InfoTaskID (void);

DLL_PUBLIC MS_BOOL MsOS_GetTaskStatus (Task_Info* pTaskInfo, TaskStatus *peTaskState);

//-------------------------------------------------------------------------------------------------
/// Get thread ID of current thread/process in OS
/// @return : current thread ID
//-------------------------------------------------------------------------------------------------
DLL_PUBLIC MS_S32 MsOS_GetOSThreadID (void);
...
DLL_PUBLIC MS_S32 MsOS_CreateMutex ( MsOSAttribute eAttribute, char *pMutexName, MS_U32 u32Flag);

DLL_PUBLIC MS_BOOL MsOS_DeleteMutex (MS_S32 s32MutexId);

DLL_PUBLIC MS_BOOL MsOS_ObtainMutex (MS_S32 s32MutexId, MS_U32 u32WaitMs);

DLL_PUBLIC MS_BOOL MsOS_ReleaseMutex (MS_S32 s32MutexId);

DLL_PUBLIC MS_BOOL MsOS_InfoMutex (MS_S32 s32MutexId, MsOSAttribute *peAttribute, char *pMutexName);


//
// Semaphore
//
DLL_PUBLIC MS_S32 MsOS_CreateSemaphore (MS_U32 u32InitCnt,
                             MsOSAttribute eAttribute,
                             char *pName);

DLL_PUBLIC MS_BOOL MsOS_DeleteSemaphore (MS_S32 s32SemaphoreId);

DLL_PUBLIC MS_BOOL MsOS_ObtainSemaphore (MS_S32 s32SemaphoreId, MS_U32 u32WaitMs);

DLL_PUBLIC MS_BOOL MsOS_ReleaseSemaphore (MS_S32 s32SemaphoreId);

DLL_PUBLIC MS_BOOL MsOS_InfoSemaphore (MS_S32 s32SemaphoreId, MS_U32 *pu32Cnt, MsOSAttribute *peAttribute, char *pSemaphoreName);


//
// Event management
//
DLL_PUBLIC MS_S32 MsOS_CreateEventGroup (char *pName);

DLL_PUBLIC MS_BOOL MsOS_DeleteEventGroup (MS_S32 s32EventGroupId);

DLL_PUBLIC MS_BOOL MsOS_SetEvent (MS_S32 s32EventGroupId, MS_U32 u32EventFlag);

DLL_PUBLIC MS_BOOL MsOS_ClearEvent (MS_S32 s32EventGroupId, MS_U32 u32EventFlag);

DLL_PUBLIC MS_BOOL MsOS_WaitEvent (MS_S32  s32EventGroupId,
...
// Timer management
//
DLL_PUBLIC MS_S32 MsOS_CreateTimer (TimerCb    pTimerCb,
                         MS_U32     u32FirstTimeMs,
                         MS_U32     u32PeriodTimeMs,
                         MS_BOOL    bStartTimer,
                         char       *pName);

DLL_PUBLIC MS_BOOL MsOS_DeleteTimer (MS_S32 s32TimerId);

DLL_PUBLIC MS_BOOL MsOS_StartTimer (MS_S32 s32TimerId);

DLL_PUBLIC MS_BOOL MsOS_StopTimer (MS_S32 s32TimerId);

DLL_PUBLIC MS_BOOL MsOS_ResetTimer (MS_S32     s32TimerId,
                         MS_U32     u32FirstTimeMs,
                         MS_U32     u32PeriodTimeMs,
                         MS_BOOL    bStartTimer);
...
DLL_PUBLIC MS_U32 MsOS_GetSystemTime (void);


DLL_PUBLIC MS_U32 MsOS_Timer_DiffTimeFromNow(MS_U32 u32TaskTimer); ///[OBSOLETE]

DLL_PUBLIC MS_U32 MsOS_Timer_DiffTime(MS_U32 u32Timer, MS_U32 u32TaskTimer); ///[OBSOLETE]
...
// Queue
//
DLL_PUBLIC MS_S32 MsOS_CreateQueue (void           *pStartAddr,
                         MS_U32         u32QueueSize,
...
DLL_PUBLIC MS_BOOL MsOS_DeleteQueue (MS_S32 s32QueueId);

DLL_PUBLIC MS_BOOL MsOS_SendToQueue (MS_S32 s32QueueId, MS_U8 *pu8Message, MS_U32 u32Size, MS_U32 u32WaitMs);

DLL_PUBLIC MS_BOOL MsOS_RecvFromQueue (MS_S32 s32QueueId, MS_U8 *pu8Message, MS_U32 u32IntendedSize, MS_U32 *pu32ActualSize, MS_U32 u32WaitMs);

DLL_PUBLIC MS_BOOL MsOS_PeekFromQueue (MS_S32 s32QueueId, MS_U8 *pu8Message, MS_U32 u32IntendedSize, MS_U32 *pu32ActualSize);

//
// Interrupt management
//
DLL_PUBLIC MS_BOOL MsOS_AttachInterrupt (InterruptNum eIntNum, InterruptCb pIntCb);

DLL_PUBLIC MS_BOOL MsOS_DetachInterrupt (InterruptNum eIntNum);

DLL_PUBLIC MS_BOOL MsOS_EnableInterrupt (InterruptNum eIntNum);

DLL_PUBLIC MS_BOOL MsOS_DebugInterrupt (InterruptNum eIntNum, IrqDebugOpt eIrqDebugOpt);

DLL_PUBLIC MS_BOOL MsOS_DisableInterrupt (InterruptNum eIntNum);

DLL_PUBLIC MS_BOOL MsOS_CompleteInterrupt (InterruptNum eIntNum);

DLL_PUBLIC MS_BOOL MsOS_In_Interrupt (void);

DLL_PUBLIC MS_U32 MsOS_DisableAllInterrupts(void);

DLL_PUBLIC MS_BOOL MsOS_RestoreAllInterrupts(MS_U32 u32OldInterrupts);

DLL_PUBLIC MS_BOOL MsOS_EnableAllInterrupts(void);
...
DLL_PUBLIC MS_U32 MsOS_CPU_DisableInterrupt (void);

DLL_PUBLIC MS_BOOL MsOS_CPU_EnableInterrupt (void);

DLL_PUBLIC MS_BOOL MsOS_CPU_RestoreInterrupt (MS_U32 u32OldInterrupts);

DLL_PUBLIC MS_BOOL MsOS_CPU_MaskAllInterrupt (void);

DLL_PUBLIC MS_BOOL MsOS_CPU_MaskInterrupt (MHAL_INTERRUPT_TYPE intr_num);

DLL_PUBLIC MS_BOOL MsOS_CPU_UnMaskInterrupt (MHAL_INTERRUPT_TYPE intr_num);

DLL_PUBLIC MS_BOOL MsOS_CPU_LockInterrupt (void);

DLL_PUBLIC MS_BOOL MsOS_CPU_UnLockInterrupt (void);

DLL_PUBLIC MS_BOOL MsOS_CPU_AttachInterrupt (MHAL_INTERRUPT_TYPE intr_num, mhal_isr_t isr, MS_U32 dat);

DLL_PUBLIC MS_BOOL MsOS_CPU_DetachInterrupt (MHAL_INTERRUPT_TYPE intr_num);

DLL_PUBLIC MS_BOOL MsOS_CPU_AttachException (MHAL_EXCEPTION_TYPE expt_num, mhal_isr_t isr, MS_U32 dat);

DLL_PUBLIC MS_BOOL MsOS_CPU_DetachExceptiont (MHAL_EXCEPTION_TYPE expt_num);

DLL_PUBLIC MS_BOOL MsOS_CPU_SetEBASE (MS_U32 u32Addr);
...
// Cache Opertation
//
DLL_PUBLIC MS_BOOL MsOS_Dcache_Flush( MS_U32 u32Start , MS_U32 u32Size );

DLL_PUBLIC MS_BOOL MsOS_Dcache_Invalidate( MS_U32 u32Start , MS_U32 u32Size );

DLL_PUBLIC MS_BOOL MsOS_Dcache_Writeback( MS_U32 u32Start , MS_U32 u32Size );

#if defined(CHIP_T12) || defined(CHIP_T8) || defined(CHIP_J2) || defined(CHIP_A2) || defined(CHIP_A5) || defined(CHIP_A3) || defined(CHIP_Edison)
DLL_PUBLIC MS_BOOL MsOS_L2Cache_Flush(void);
DLL_PUBLIC MS_BOOL MsOS_L2Cache_Read(void);
#endif

//
// CPU relative Operation
//
DLL_PUBLIC void MsOS_Sync(void);
...
DLL_PUBLIC MS_BOOL MDrv_SYS_Info(IO_Sys_Info_t* SysInfo);
// MPool Operation
DLL_PUBLIC void MsOS_MPool_SetDbgLevel(MsOSMPool_DbgLevel DbgLevel);
DLL_PUBLIC MS_BOOL MsOS_MPool_IsInitialized(void);
DLL_PUBLIC MS_BOOL MsOS_MPool_Init(void);
DLL_PUBLIC MS_BOOL MsOS_MPool_Get(void** pAddrVirt, MS_U32* pu32AddrPhys, MS_U32* pu32Size, MS_BOOL bNonCache);
DLL_PUBLIC MS_BOOL MsOS_MPool_Close(void);
DLL_PUBLIC MS_U32 MsOS_MPool_VA2PA(MS_U32 pAddrVirt);
DLL_PUBLIC MS_U32 MsOS_MPool_PA2KSEG1(MS_U32 pAddrPhys);
DLL_PUBLIC MS_U32 MsOS_MPool_PA2KSEG0(MS_U32 pAddrPhys);
DLL_PUBLIC MS_BOOL MsOS_MPool_Dcache_Flush(MS_U32 pAddrVirt, MS_U32 u32Size); // the input address should be user mode cacheable address
DLL_PUBLIC MS_BOOL MsOS_MPool_Dcache_Flush_All(void);
DLL_PUBLIC MS_BOOL MsOS_MPool_Boundary_Protect_Mapping(MS_U8 u8MiuSel, MS_U32 u32Offset, MS_U32 u32MapSize, unsigned int bNonCache);
DLL_PUBLIC MS_BOOL MsOS_MPool_Mapping(MS_U8 u8MiuSel, MS_U32 u32Offset, MS_U32 u32MapSize, MS_BOOL bNonCache);
DLL_PUBLIC MS_BOOL MsOS_MPool_Mapping_Dynamic(MS_U8 u8MiuSel, MS_U32 u32Offset, MS_U32 u32MapSize, MS_BOOL bNonCache);
DLL_PUBLIC MS_BOOL MsOS_MPool_Kernel_Detect(MS_U32 *lx_addr, MS_U32 *lx_size, MS_U32 *lx2_addr, MS_U32 *lx2_size);
DLL_PUBLIC MS_BOOL MsOS_MPool_SetWatchPT(MS_U32 u32Addr, MS_U32 u32ASID, MS_U8 u8Global, MS_U8 u8WType, MS_U32 u32Mask);
DLL_PUBLIC MS_BOOL MsOS_MPool_GetWatchPT(char* str);

#define MsOS_MPool_PA2VA MsOS_MPool_PA2KSEG1

DLL_PUBLIC MS_U32 MsOS_Mapping_SharedMem(MS_U32 u32ShmSize,MS_BOOL *bInit);

DLL_PUBLIC MS_S32 MsOS_CreateNamedMutex(MS_S8 *ps8MutexName);
DLL_PUBLIC MS_BOOL MsOS_LockMutex(MS_S32 u32Index,int flag);
DLL_PUBLIC MS_BOOL MsOS_UnlockMutex(MS_S32 u32Index,int flag);
DLL_PUBLIC MS_BOOL MsOS_DeleteNamedMutexbyIndex(MS_S32 u32Index);
...
DLL_PUBLIC MS_BOOL MsOS_SHM_Init(void);
DLL_PUBLIC MS_BOOL MsOS_SHM_GetId(MS_U8* pu8ClientName, MS_U32 u32BufSize, MS_U32* pu32ShmId, MS_U32* pu32Addr, MS_U32* pu32BufSize, MS_U32 u32Flag);
DLL_PUBLIC MS_BOOL MsOS_SHM_FreeId(MS_U8* pu8ClientName, MS_U32 u32ShmId);
#if defined (MSOS_TYPE_NOS)
DLL_PUBLIC MS_BOOL MsOS_MEM_GetId(MS_U8* pu8ClientName, MS_U32 u32BufSize, MS_U32* pu32ShmId, MS_U32* pu32Addr, MS_U32* pu32BufSize, MS_U32 u32Flag);
#endif
...
//-------------------------------------------------------------------------------------------------
DLL_PUBLIC MS_U32 MsOS_VA2PA(MS_U32 addr);
DLL_PUBLIC MS_U32 MsOS_PA2KSEG0(MS_U32 addr);
DLL_PUBLIC MS_U32 MsOS_PA2KSEG1(MS_U32 addr);
DLL_PUBLIC MS_U32 MsOS_PA2BA(MS_PHYADDR u32PhyAddr);
DLL_PUBLIC MS_PHYADDR MsOS_BA2PA(MS_U32 u32Addr);
DLL_PUBLIC void MsOS_FlushMemory(void);
DLL_PUBLIC void MsOS_ReadMemory(void);
...
// debug
//-------------------------------------------------------------------------------------------------
extern DLL_PUBLIC void MsOS_RegMyDbg(void);                                        ///< MsOS debug register itself debug
...
extern DLL_PUBLIC MS_BOOL MsOS_Dbg_ParseCmd(char *Cmd, MS_U32 u32CmdLen);

///< register your main menu here
extern DLL_PUBLIC MS_BOOL MsOS_Dbg_Regist(const char *Func_Name, const char *Func_Help, MS_DBG_LINK *pAryDbgLink);

///< for user to handle their own sub menu
extern DLL_PUBLIC MS_BOOL MsOS_Dbg_ExecuteSubCB(const char *Func_Name, int argc, char *argv[], MS_DBG_LINK *pAryDbgLink);

...
void MsApiDisplay_Logo(MS_U8 u8logoGopIdx, GFX_DrawRect *rect,GFX_BufferInfo *src_info,GFX_BufferInfo *dst_info, MS_BOOL bHorStretch, MS_BOOL bVerStretch,MS_U16 u16DispX,MS_U16 u16DispY)
{
    ...
    U32* u32SrcAddr = MsOS_PA2KSEG1(src_info->u32Addr);
	U32* u32DestAddr = MsOS_PA2KSEG1(dst_info->u32Addr);
	U32 u32BackColor;

    if(u32SrcAddr == NULL || u32DestAddr == NULL)
    {
    ...
    U32 i=0;
    U32 PointCnt;
	//u32BackColor = 0x80318031;
    u32BackColor = u32SrcAddr[0];
	UBOOT_DEBUG("u32SrcAddr : 0x%x \n",u32SrcAddr[0]);

	if(bLogoIsBmp == 1)
	{
	    PointCnt = (dst_info->u32Width)*(dst_info->u32Height);
	}
	else
	{
	    u32BackColor = (u32BackColor&0x00FF00FF) | ((u32BackColor&0xFF000000)>>16) | ((u32BackColor&0x0000FF00)<<16);
	    PointCnt = ((dst_info->u32Width)*(dst_info->u32Height))/2;
	}
	UBOOT_DEBUG("u32BackColor : 0x%x \n",u32BackColor);
	UBOOT_DEBUG("u32Width : 0x%x \n",dst_info->u32Width);
	UBOOT_DEBUG("u32Height : 0x%x \n",dst_info->u32Height);
    for (i=0;i<PointCnt;i++)
    {
        *u32DestAddr = u32BackColor;
        u32DestAddr++;
    }
#endif
	...
	if(bLogoIsBmp == 1)
	{
        src_info->u32Pitch =src_info->u32Pitch>>2;
        dst_info->u32Pitch =dst_info->u32Pitch>>2;
	}
	else
	{
    ...
    if(bLogoIsBmp == 1)
	    mirrorMode = 2;    //bmp logo neet to mirror v on
...
void MsApiOsdCreate(MS_U8 u8logoGopIdx, GFX_Block* pBlk, U32 u32GopBuffer)
{
    ...
    gu32CanvasBuffer=u32GopBuffer+(pBlk->width* g_IPanel.Height()*2);
    ...
    u16TempAddr = (U16*)PA2NVA(u32GopBuffer);

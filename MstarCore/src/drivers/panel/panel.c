...
PanelType  tDACOUT1024X768P_60[] =
{
    {//Dac Out for U3 bringup
        ...
        0,            // 6 + 29 ,       //WORD m_wPanelVStart;              //PANEL_VSTART             (PANEL_VSYNC_WIDTH + PANEL_VSYNC_BACK_PORCH)
...
PanelType  tDACOUT1280X1024P_60[] =
{
    {//Dac Out for U3 bringup
...
PanelType  tDACOUT1440X900P_60[] =
{
    {//Dac Out for U3 bringup
        ...
        0,            //6 + 25,       //WORD m_wPanelVStart;              //PANEL_VSTART             (PANEL_VSYNC_WIDTH + PANEL_VSYNC_BACK_PORCH)
...
PanelType  tDACOUT1600X1200P_60[] =
{
    {
        ...
        304,         //BYTE m_ucPanelHSyncBackPorch;     //PANEL_HSYNC_BACK_PORCH
        ...
        0,            //(3+46),       //WORD m_wPanelVStart;              //PANEL_VSTART             (PANEL_VSYNC_WIDTH + PANEL_VSYNC_BACK_PORCH)
...
PanelType * PanelIndexTbl[] =
{
    ...
    tDACOUT1024X768P_60,        // DAC 1280*768P 60Hz
    tDACOUT1280X1024P_60,       // DAC 1280*1024P 60Hz
    tDACOUT1440X900P_60,        // DAC 1440*900P 60Hz
    tDACOUT1600X1200P_60,       // DAC 1600*1200P 60Hz
...
{
    char  filepath[256];
    MS_U32 lut_phy_addr  = 0;
    MS_U32 lut_bus_addr = 0;
    MS_U32 lut_temp_addr = 0;
    MS_U32 u32fileSize;
    DeMuraBinHeader  BinHeader;

    printf("\n");
    if (get_addr_from_mmap(DEMURA_MMAP_ID, (U32 *)&lut_phy_addr) == -1)
    {
        UBOOT_ERROR("get %s mmap fail\n",DEMURA_MMAP_ID);
        return FALSE;
    }
    lut_bus_addr = PA2BA(lut_phy_addr);
    lut_temp_addr = DEMURA_LUT_TMP_LOAD_ADDR;

    if (check_str_resume() == 1)
    {
        memcpy((void *)&BinHeader, (void *)lut_bus_addr, sizeof(DeMuraBinHeader));
        if (MDrv_DeMura_Check_HeaderCRC(&BinHeader, TRUE) == FALSE)
        {
            DEBUG_DEMURA("Calculate Demura Bin header CRC fail\n");
    ...
    {
        vfs_mount(CONFIG);
        snprintf(filepath, sizeof(filepath),"%s/demura/lut.bin", CONFIG_PATH);
        u32fileSize = vfs_getsize(filepath);
        DEBUG_DEMURA("Filepath : %s, fileSize = 0x%x\n", filepath, u32fileSize);

        if (vfs_read((void *)&BinHeader, filepath, 0, sizeof(DeMuraBinHeader)) == -1)
        {
            UBOOT_ERROR("vfs_read Demura Bin header fail ...>>>\n");
            return FALSE;
        }
        MDrv_DeMura_Dump_BinInfo(&BinHeader);
        if (MDrv_DeMura_Check_HeaderCRC(&BinHeader, FALSE) == FALSE)
        {
            UBOOT_ERROR("Calculate Demura Bin header CRC fail\n");
            return FALSE;
        }
        if (MDrv_DeMura_Is_Support(&BinHeader, panel_data) == FALSE)
        {
            UBOOT_ERROR("This Demura Bin(%s) do Not Support this panel\n", filepath);
            return FALSE;
        }

        if (vfs_read((void *)lut_temp_addr, (char *)filepath, 0, u32fileSize) ==  -1)
        {
            UBOOT_ERROR("vfs_read Demura Bin fail ...>>>\n");
            return FALSE;
        }
        if (MDrv_DeMura_Check_AllBinCRC(lut_temp_addr) == FALSE)
        {
            UBOOT_ERROR("Calculate All Demura Bin CRC fail\n");
            return FALSE;
        }

        if (MDrv_DeMura_LoadData(&BinHeader, lut_bus_addr, lut_temp_addr) == FALSE)
        {
            UBOOT_ERROR("Load Demura Data fail\n");
            return FALSE;
        }
    }

    if (MDrv_DeMura_RegisterTableLoad(&BinHeader, (lut_bus_addr + BinHeader.u32RegDataAddr)) == FALSE)
    {
        UBOOT_ERROR("MDrv_DeMura_RegisterTableLoad error\n");
        return FALSE;
    }

    DEBUG_DEMURA("layer data is loaded on phy_addr : 0x%x\n", (lut_phy_addr + BinHeader.u32LayerDataAddr));
    MDrv_DeMura_AutoDownload_Setup(&BinHeader, (lut_phy_addr + BinHeader.u32LayerDataAddr));
    MDrv_DeMura_EnableDemura(TRUE);
    return TRUE;
}
#endif

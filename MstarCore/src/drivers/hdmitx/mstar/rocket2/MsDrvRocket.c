...
static MS_BOOL SelectProperTimingFromEDID(const MS_U8 *pTimingList, const MS_U8 TimingListSize, MS_U8 *enSelectedTiming)
{
    ...
    {
        printf("!!!!! Error: Timing list is empty.\n");
        return FALSE;
    }

    for(i = 0; i < TimingListSize; i++)
    {
        enTiming = pTimingList[i];
        ...
        if(TRUE == g_bEDID4K2KReady)
        {
            ...
            {
                for(j = 0; j < EDID_VIC_4K2K_NUM; j++)
                {
                    UBOOT_DEBUG("[%s:%d] enHDMITx_4K2K[%d]=%d\n", __FUNCTION__, __LINE__, j, g_u8HDMITx4K2K[j]);
                    if(g_u8HDMITx4K2K[j] == enEdid4k2kVIC)
                    {
        ...
        if(TRUE == g_bEDIDReady)
        {
            ...
            for(j = 0; j < EDID_VIC_NUM; j++)
            {
                UBOOT_DEBUG("[%s:%d] g_u8HDMITx[%d]=%d\n", __FUNCTION__, __LINE__, j, g_u8HDMITx[j]);
                if(g_u8HDMITx[j] == 0)
                {
                    break;
                }

                if(((g_u8HDMITx[j]&0x7F) == u8Target_4x3) || ((g_u8HDMITx[j]&0x7F) == u8Target_16x9))
                {
                    *enSelectedTiming = enTiming;
                    return TRUE;
                }
...
static E_MAPI_RAPTORS_SUPPORT_FORMAT checkReciverSupportFormat(int index)
{
    int idx = 0;
    ...
    if(TRUE == g_bEDIDReady)
    {
        ...
        switch(index)
        {
            ...
            case E_MAPI_ROCKY_RES_4K2Kp_60Hz:
                ...
                break;
            default:
                UBOOT_DEBUG("Invalid VIC Type(%d)\n", index);
                break;
        }
        ...
        for(idx = 0; idx < EDID_VIC_NUM; idx++)
        {
            ...
            {
                if(g_u8HDMITx[idx] & 0x80)
                {
                    if(!strcmp(getenv("hdmitx_edid"), "2"))
                    {
                        return E_MAPI_RAPTORS_RESOLUTION_BESTSUPPORT_SUCCESS;
                    }
                    else
                    {
                        return E_MAPI_RAPTORS_RESOLUTION_SUPPORT_SUCCESS;
                    }
                }
                else
                {
    ...
    if(TRUE == g_bEDID4K2KReady)
    {
        ...
        switch(index)
        {
            ...
            case E_MAPI_ROCKY_RES_4K2Kp_24Hz:
                ...
                break;
            default:
                UBOOT_DEBUG("Invalid VIC 4K2K Type(%d)\n", index);
                break;
        }
        //VSBD Debug Msg
        for(idx = 0; idx < HDMITx_VIDEO_4k2k_24Hz_SMPTE; idx++)
        {
            if(g_u8HDMITx4K2K[idx] == 0)
            {
                break;
            }
            else if(g_u8HDMITx4K2K[idx] == enEdid4k2kVIC)
            {
                return E_MAPI_RAPTORS_RESOLUTION_SUPPORT_SUCCESS;
            }
        }
    ...
    return E_MAPI_RAPTORS_RESOLUTION_NOT_SUPPORT;
}
...
MS_BOOL msHdmitx_Rocket_Init(EN_MAPI_DEVICE_ROCKY_VIDEO_TIMING outPutType )
{
    ...
    if((pEnv != NULL) && (u8_RxStatus))
    {
        ...
        if(!(strcmp(pEnv, "2")))
        {
            ...
            {
                if( checkReciverSupportFormat((int)SelectedTiming) == E_MAPI_RAPTORS_RESOLUTION_BESTSUPPORT_SUCCESS)
                {
        ...
        else if(!(strcmp(pEnv, "3")))
        {
            ...
            {
                if(checkReciverSupportFormat((int)SelectedTiming) == E_MAPI_RAPTORS_RESOLUTION_SUPPORT_SUCCESS)
                {
        ...
        if(!(strcmp(pEnv, "1")) || ((bBestSupportTiming == FALSE) && !(strcmp(pEnv, "2"))))
        {
            ...
            status = checkReciverSupportFormat((int)outPutType);

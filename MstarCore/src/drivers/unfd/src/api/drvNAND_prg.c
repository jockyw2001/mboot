						   ...
						   U32 u32_SrcPhyAddr, U32 u32_SectorCnt )
{
	...
	{
		nand_debug(UNFD_DEBUG_LEVEL, 1, "buffer is not cache line alignment:%lX\n", u32_SrcPhyAddr);
...
int MDrv_Nand_init(unsigned char *nniData, unsigned char *pniData)
{
    ...
    if (u32_Ret == UNFD_ST_SUCCESS || (u32_Ret == UNFD_ST_ERR_NO_CIS))
    {
        ...
        else
        {
            ...
            if (u32_Ret != UNFD_ST_SUCCESS)
            {
                nand_debug(UNFD_DEBUG_LEVEL_ERROR, 1, "ERROR, write cis fail: %lX\n", u32_Ret);
                drvNAND_unlock_sem();
                return FALSE;
            }
            else
            {
                ...
                if (u32_Ret != UNFD_ST_SUCCESS)
                {
                    nand_debug(UNFD_DEBUG_LEVEL_ERROR, 1, "ERROR, init fail: %lX\n", u32_Ret);
                    drvNAND_unlock_sem();
                    return FALSE;
                }
            }
...
extern int board_ftl_init(bd_t *bis);
extern int nand_ftl_Init(struct ftl* ftl);

    ...
    for (u16_PBA = 0; u16_PBA < 10; u16_PBA++) {
        ...
        **/
		...
		if(pNandDrv->u8_CellType == NAND_CellType_SLC)  // SLC write dummy to IBBT page
		{
			memset(au8_PageBuf, '\0', pNandDrv->u16_PageByteCnt);
			memset(au8_SpareBuf, 0xFF, pNandDrv->u16_SpareByteCnt);
			u32_Err = NC_WritePages(u32_PageIdx+ga_tPairedPageMap[2].u16_LSB, au8_PageBuf, au8_SpareBuf, 1);
			if (u32_Err != UNFD_ST_SUCCESS) {
        ...
        */
		...
		while(ParamMIUSize > 0)
		{
			...
			u32_Err = NC_WriteSectors(ProgramMiuPage,0,(U8 *)((ProgramMiuAddr+(ProgramMiuPage-10-u32_PageIdx)*(U32)pNandDrv->u16_SectorByteCnt)), au8_SpareBuf,1);
			if (u32_Err != UNFD_ST_SUCCESS)
			{
...
int drvNAND_ReadCISBlk(U32 u32_off, U8* pu8_DataBuf, u16 u16_phySpareByteCnt)
{
    ...
    //Read page1~page9 with normal ecc type
    for(i = 1 ; i < ProgramMiuPage; i ++)
    {
...
{    
    ...
    u8_Randomizer = pNandDrv->u8_RequireRandomizer;
...
//    nand_debug(0, 1, "Begin Refresh all Blk \n");
    if(refresh)
    {
        for(u16_PBA = 0xA; u16_PBA < pNandDrv->u16_BlkCnt; u16_PBA ++)
        {
            ...
            if(u16_PBA < u16_MbootPartEndPBA)
                pNandDrv->u8_RequireRandomizer = 0;
            else
                pNandDrv->u8_RequireRandomizer = u8_Randomizer;
    //        nand_debug(0, 1, "Read Blk 0x%X\n", u16_PBA);
...
U32 drvNAND_WriteLoader(int LOADERBLOCK, U8* u8LDRPos, int size)
{
    ...
    while(size > 0) {
        printf("[LDR] u32Row=%ld u8LDRPos=0x%08lx u8Spare[1]=0x%08lx\n", u32Row, (U32)u8LDRPos, u8Spare[1]);
        ...
        {
        	printf("[LDR] NC_WritePages failed v=%lu,u32Row=%ld u8APPPos=0x%08lx \n", v, u32Row, (U32)u8LDRPos);
		}

...
#else
    if(get_value_from_env("E_MMAP_ID_OAD_DOWNLOAD_ADR", DOWNLOAD_BUFFER_ADR_OAD,&u32DownloadOadBuffADR)!=0)
    {
...
#elif defined(CONFIG_KENYA) || ((defined(CONFIG_KERES) || defined(CONFIG_KIRIN)||defined(CONFIG_KRATOS)) && ENABLE_DVB_C_SYS)
            sprintf(buffer, "dmx_init 0");
#elif (defined(CONFIG_K5TN) && ENABLE_DVB_T2_SYS)
            sprintf(buffer, "dmx_init 0");
...
#else
    if(get_value_from_env(E_MMAP_ID_DMX_SECBUF_ADR, DMX_SEC_BUFFER_ADR,&u32DmxSecBuffADR)!=0)
    {
        printf("get %s mmap fail\n",E_MMAP_ID_DMX_SECBUF_ADR);
        return -1;
    }
    if(get_value_from_env(E_MMAP_ID_DMX_SECBUF_LEN, NO_DEFAULT_MMAP_VALUE,&u32DmxSecBuffLen)!=0)
    {

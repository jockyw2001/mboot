...
int MsDrv_JpdInit(void)
{
    ...
    //E_MMAP_ID_JPD_READ 
    if(get_value_from_env(E_MMAP_ID_JPD_READ_ADR, MMAP_E_MMAP_ID_JPD_READ_ADDR, &u32ReadBuffPA)!=0)
    {
        UBOOT_ERROR("get %s mmap fail\n",E_MMAP_ID_JPD_READ_ADR);
        return -1;
    }

    if(get_value_from_env(E_MMAP_ID_JPD_READ_LEN, MMAP_E_MMAP_ID_JPD_READ_SIZE, &u32ReadBuffSize)!=0)
    {
    ...
    //E_MMAP_ID_JPD_WRITE
    if(get_value_from_env(E_MMAP_ID_JPD_WRITE_ADR, MMAP_E_MMAP_ID_JPD_WRITE_ADDR, &u32WriteBuffPA)!=0)
    {
    ...
    if(get_value_from_env(E_MMAP_ID_JPD_WRITE_LEN, MMAP_E_MMAP_ID_JPD_WRITE_SIZE, &u32WriteBuffSize)!=0)
    {
    ...
    if(get_value_from_env(E_MMAP_ID_PHOTO_INTER_ADR, MMAP_E_MMAP_ID_PHOTO_INTER_ADDR, &u32InterBuffPA)!=0)
    {
    ...
    if(get_value_from_env(E_MMAP_ID_PHOTO_INTER_LEN, MMAP_E_MMAP_ID_PHOTO_INTER_SIZE, &u32InterBuffSize)!=0)
    {
        UBOOT_ERROR("get %s mmap fail\n",E_MMAP_ID_PHOTO_INTER_LEN);
        return -1;
    }

    if((u32ReadBuffPA == 0)   ||(u32WriteBuffPA == 0)   || (u32InterBuffPA == 0) ||  

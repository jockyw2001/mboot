...
void MHal_EMAC_Power_On_Clk(void)
{
    ...
    /* Init ePHY */
    //Set to HV mode
    MHal_EMAC_WritReg8(0x0032, 0xC7, 0xC0);
    uRegVal = MHal_EMAC_ReadReg8(0x000e, 0x60);
    uRegVal &= 0xfe;
    MHal_EMAC_WritReg8(0x000e, 0x60, uRegVal);

    MHal_EMAC_WritReg8(0x0032, 0xB4, 0x02);     //wriu      0x0032B4  0x02     // gain shift
    MHal_EMAC_WritReg8(0x0032, 0x4F, 0x02);     //wriu      0x00324F  0x02     // det max
    MHal_EMAC_WritReg8(0x0032, 0x51, 0x01);     //wriu      0x003251  0x01     // det min
    MHal_EMAC_WritReg8(0x0032, 0x77, 0x18);     //wriu      0x003277  0x18     // snr len (emc noise)
//    MHal_EMAC_WritReg8(0x0032, 0x43, 0x15);  //// wriu      0x003243  0x15     // snr k value

//    MHal_EMAC_WritReg8(0x0032, 0xDE, 0x59);  //// wriu      0x0032DE  0x59     // PLL DIV
//    MHal_EMAC_WritReg8(0x0032, 0xF4, 0x20);  //// wriu      0x0032F4  0x20     // function PLL MOD
    MHal_EMAC_WritReg8(0x0031, 0x72, 0xA0);     //wriu      0x003172  0xa0     // lpbk_enable set to 0

    MHal_EMAC_WritReg16(0x0032, 0xFC, 0x0000);  //wriu  -w  0x0032fc  0x0000   // Power-on LDO
    MHal_EMAC_WritReg8(0x0032, 0xB7, 0x17);     //wriu      0x0032b7  0x17     // Power-on ADC**
    MHal_EMAC_WritReg8(0x0032, 0xCB, 0x11);     //wriu      0x0032cb  0x11     // Power-on BGAP
    MHal_EMAC_WritReg8(0x0032, 0xCC, 0x20);     //wriu      0x0032cc  0x20     // Power-on ADCPL
    MHal_EMAC_WritReg8(0x0032, 0xCD, 0xD0);     //wriu      0x0032cd  0xd0     // Power-on ADCPL
    MHal_EMAC_WritReg8(0x0032, 0xD4, 0x00);     //wriu      0x0032d4  0x00     // Power-on LPF_OP
    MHal_EMAC_WritReg8(0x0032, 0xB9, 0x40);     //wriu      0x0032b9  0x40     // Power-on LPF
    MHal_EMAC_WritReg8(0x0032, 0xBB, 0x05);     //wriu      0x0032bb  0x05     // Power-on REF
    MHal_EMAC_WritReg16(0x0033, 0x3A, 0x0003);  //wriu  -w  0x00333a  0x0003   // PD_TX_IDAC, PD_TX_LD = 0

    MHal_EMAC_WritReg8(0x0032, 0x3B, 0x01);     //wriu      0x00323b  0x01     // SAR CLK
    MHal_EMAC_WritReg8(0x0033, 0xA1, 0xC0);     //wriu      0x0033a1  0xc0     // PD_SADC, EN_SAR_LOGIC**
    MHal_EMAC_WritReg8(0x0033, 0x8A, 0x01);     //wriu      0x00338a  0x01     // CLKO_ADC_SEL**
    MHal_EMAC_WritReg8(0x0032, 0xC4, 0x44);     //wriu      0x0032c4  0x44     // TEST**
    MHal_EMAC_WritReg8(0x0033, 0x80, 0x30);     //wriu      0x003380  0x30     // timer off

    MHal_EMAC_WritReg8(0x0033, 0xC5, 0x00);     //wriu      0x0033c5  0x00     // 100gat
    MHal_EMAC_WritReg8(0x0033, 0x30, 0x43);     //wriu      0x003330  0x43     // 200gat

    MHal_EMAC_WritReg8(0x0033, 0x39, 0x41);     //wriu      0x003339  0x41     // en_100t_phase;  [6] save2x_tx
/// not yet wriu  -w  0x0032c2  0x7000   // 100T TX performance (TEST_TX)

    MHal_EMAC_WritReg8(0x0031, 0x79, 0xD0);     //wriu      0x003179  0xd0     // prevent packet drop by inverted waveform & 10 gain to D
    MHal_EMAC_WritReg8(0x0031, 0x77, 0x5A);     //wriu      0x003177  0x5a     //

    MHal_EMAC_WritReg8(0x0031, 0x2D, 0x7C);     //wriu      0x00312d  0x7c     // disable eee

//10T waveform
    MHal_EMAC_WritReg8(0x0033, 0xE8, 0x06);     //wriu      0x0033e8  0x06
    MHal_EMAC_WritReg8(0x0031, 0x2B, 0x00);     //wriu      0x00312b  0x00
    MHal_EMAC_WritReg8(0x0033, 0xE8, 0x00);     //wriu      0x0033e8  0x00
    MHal_EMAC_WritReg8(0x0031, 0x2B, 0x00);     //wriu      0x00312b  0x00


    MHal_EMAC_WritReg8(0x0033, 0xE8, 0x06);     //wriu      0x0033e8  0x06       // shadow_ctrl
    MHal_EMAC_WritReg8(0x0031, 0xAA, 0x14);     //wriu      0x0031aa  0x14       // tin17_s2
    MHal_EMAC_WritReg16(0x0031, 0xAC, 0x1414);  //wriu  -w  0x0031ac  0x1414     // tin18_s2
    MHal_EMAC_WritReg16(0x0031, 0xAE, 0x1414);  //wriu  -w  0x0031ae  0x1414     // tin19_s2


    MHal_EMAC_WritReg8(0x0033, 0xE8, 0x00);     //wriu      0x0033e8  0x00       // shadow_ctrl
    MHal_EMAC_WritReg16(0x0031, 0xAA, 0x2814);  //wriu  -w  0x0031aa  0x2814     // tin17_s2



//speed up timing recovery
    MHal_EMAC_WritReg8(0x0032, 0xF5, 0x02);     //wriu      0x0032f5  0x02

// Signal_det k
    MHal_EMAC_WritReg8(0x0032, 0x0F, 0xC9);     //wriu      0x00320f  0xc9

// snr_h
    MHal_EMAC_WritReg8(0x0032, 0x89, 0x50);     //wriu      0x003289  0x50
    MHal_EMAC_WritReg8(0x0032, 0x8B, 0x80);     //wriu      0x00328b  0x80
    MHal_EMAC_WritReg8(0x0032, 0x8E, 0x0E);     //wriu      0x00328e  0x0e
    MHal_EMAC_WritReg8(0x0032, 0x90, 0x04);     //wriu      0x003290  0x04

// 10t_8bt
    MHal_EMAC_WritReg8(0x0031, 0xB4, 0x5A);     //wriu      0x0031b4  0x5a     // def 32, counterwidth_max
    MHal_EMAC_WritReg8(0x0031, 0xB6, 0x50);     //wriu      0x0031b6  0x50     // def 28, counter_width_h (normal)
    MHal_EMAC_WritReg8(0x0031, 0xFF, 0x1A);     //wriu      0x0031ff  0x1a     // def 10, counter_width_h (lp)

// 10t_gain
    MHal_EMAC_WritReg8(0x0031, 0x79, 0xD0);     //wriu      0x003179  0xd0
...
{
    MHal_EMAC_WritReg8(0x0032, 0x04, 0x31);     //wriu      0x003204  0x31     // Reset analog

    udelay(100);                                //wait 100

    MHal_EMAC_WritReg16(0x0032, 0xFC, 0x0102);  //wriu  -w  0x0032fc  0x0102   // Power-on LDO
    MHal_EMAC_WritReg8(0x0032, 0xB7, 0x17);     //wriu      0x0032b7  0x17     // Power-on ADC**
    MHal_EMAC_WritReg8(0x0032, 0xCB, 0x13);     //wriu      0x0032cb  0x13     // Power-on BGAP
    MHal_EMAC_WritReg8(0x0032, 0xCC, 0x30);     //wriu      0x0032cc  0x30     // Power-on ADCPL
    MHal_EMAC_WritReg8(0x0032, 0xCD, 0xD8);     //wriu      0x0032cd  0xd8     // Power-on ADCPL
    MHal_EMAC_WritReg8(0x0032, 0xD4, 0x20);     //wriu      0x0032d4  0x20     // Power-on LPF_OP
    MHal_EMAC_WritReg8(0x0032, 0xB9, 0x41);     //wriu      0x0032b9  0x41     // Power-on LPF
    MHal_EMAC_WritReg8(0x0032, 0xBB, 0x45);     //wriu      0x0032bb  0x45     // Power-on REF

    MHal_EMAC_WritReg16(0x0033, 0x3A, 0x03F3);  //wriu  -w  0x00333a  0x03f3   // PD_TX_IDAC, PD_TX_LD = 0

    MHal_EMAC_WritReg8(0x0033, 0xA1, 0x30);     //wriu      0x0033a1  0x30     // PD_SADC, EN_SAR_LOGIC**


    MHal_EMAC_WritReg8(0x0033, 0xC5, 0x40);     //wriu      0x0033c5  0x40     // 100gat
    MHal_EMAC_WritReg8(0x0033, 0x30, 0x53);     //wriu      0x003330  0x53     // 200gat

    MHal_EMAC_WritReg8(0x0032, 0x04, 0x11);     //wriu      0x003204  0x11     // Release reset analog
}

...
void HalSdio_Platform_InitChiptop(void)
{
	...
	FCIE_RIU_16_OF(CHIPTOP_7Bh, REG_SDIO_CONFIG_MSK);
		...
		FCIE_RIU_16_OF(CHIPTOP_6Fh, BIT06|BIT07);

		FCIE_RIU_16_OF(CHIPTOP_30h, BIT08);

		FCIE_RIU_16_OF(CHIPTOP_12h,0x77);
		
		FCIE_RIU_16_OF(CHIPTOP_7Bh, BIT05);
		FCIE_RIU_16_ON(CHIPTOP_7Bh, BIT04);    //mode 1
	
		FCIE_RIU_16_ON(CHIPTOP_43h, BIT11|BIT12|BIT13|BIT14|BIT15);

	// MIU select by SDIO
		FCIE_RIU_16_ON(MIU2_79, BIT10);
...
void HalSdio_SwitchPad(unsigned char u32Mode)
{
	...
	// sdio
	FCIE_RIU_16_OF(FCIE_DDR_MODE, 0xffff);		//clear all
	...
	switch(u32Mode)
	{
		...
		case SDIO_MODE_GPIO_PAD_BPS:
			...
			FCIE_RIU_16_ON(FCIE_DDR_MODE, BIT01|BIT02);
		...
		case SDIO_MODE_GPIO_PAD_SDR:
			...
			FCIE_RIU_16_ON(FCIE_DDR_MODE, BIT_FALL_LATCH|BIT_PAD_IN_SEL_SD|BIT01|BIT02|BIT03|BIT_FEEDBACK_CLK2);
...
U8 HalSdio_Platform_ClearEvent(U16 nReq)
{
	...
	while((FCIE_RIU_R16(FCIE_MIE_EVENT)& nReq) != 0x00)
	{
		...
		if (u16Tmp==0)
		{
			printk("Error!!! Can not clear MIE event:%x.\r\n",nReq);
			return(1);
		}
		else
		{

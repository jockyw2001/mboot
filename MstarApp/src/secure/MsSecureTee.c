...
#else
    #define _PA2VA(x)   0x80000000 | x
    #define _PA2NVA(x)  0xa0000000 | x
    #define _VA2PA(x)   ~(0xf0000000) & x
#endif
//-------------------------------------------------------------------------------------------------
//  Global Variables
//-------------------------------------------------------------------------------------------------
...
void _CheckNuttxAddr(unsigned int uitee_start_paddr)
{
    ...
    if(puEnvNuttxAddr!=NULL)
    {
        u32EnvNuttxAddr=simple_strtol(puEnvNuttxAddr, NULL, 16);
...
int TEE_BACKUP(U32 u32NuttxPhyAddr)
{
    ...
    // TEE BACK UP
    UBOOT_DEBUG("*****_PA2NVA(NUTTX_BACKUP_HEADER) =%lx*****\n",_PA2NVA(u32NuttxPhyAddr + 0x100 - 0x20));
    UBOOT_DUMP((_PA2NVA(u32NuttxPhyAddr + NUTTX_HEADER_OFFSET)), 0x30);

    ST_BACKUP_HEADER_INFO BackUpInfo;

    memcpy((void*)&BackUpInfo,(void*)_PA2NVA(u32NuttxPhyAddr + NUTTX_HEADER_OFFSET),sizeof(ST_BACKUP_HEADER_INFO));
    ...
    if (get_value_from_env(E_MMAP_ID_NUTTX_MEM_ADR, NO_DEFAULT_MMAP_VALUE, &u32NuttxStart) != 0)
    {
        UBOOT_ERROR("get %s mmap fail\n",E_MMAP_ID_NUTTX_MEM_ADR);
        return -1;
    }
    if (get_value_from_env(E_MMAP_ID_NUTTX_MEM_LEN, NO_DEFAULT_MMAP_VALUE, &u32NuttxLen) != 0)
    {
        UBOOT_ERROR("get %s mmap fail\n",E_MMAP_ID_NUTTX_MEM_LEN);
        return -1;
    }
    u32Nuttx_End_Addr = u32NuttxStart + u32NuttxLen - TEEStoreSize - NUTTX_BOOT_CONFIG_SIZE;//Nuttx Dram End
    ...
    UBOOT_DEBUG("u32Nuttx_End_Addr = 0x%x\n",u32Nuttx_End_Addr);
    ...
    if(TEEStoreSize > (u32TEEBackUpSize + u32DataSectionLength + u32TASectionLength))//Backup Data Section
    {
        ...
        u32DataSectionStoreAddr = u32Nuttx_End_Addr + u32TEEBackUpSize;
        UBOOT_DEBUG("STR Boot Magic ID Address = 0x%x\n",           u32Nuttx_End_Addr);
        UBOOT_DEBUG("Back Up Boot Config Address = 0x%x\n",         u32Nuttx_End_Addr + STR_BootFlagSize + NConfigBKSize);
        UBOOT_DEBUG("Back Up Customer Key Bank Address  = 0x%x\n",  u32Nuttx_End_Addr + STR_BootFlagSize + NConfigBKSize + NUTTX_BOOT_CONFIG_SIZE);
        //TA section range
...
int STR_BACKUP(U32 u32NuttxPhyAddr)
{
    ...
    u32AesAddr=simple_strtoul(getenv("E_MMAP_ID_NUTTX_STR_STATUS_ADR"),NULL,16);
#else
    u32AesAddr=simple_strtoul(getenv(E_MMAP_ID_HW_AES_BUF_ADR),NULL,16);
#endif
    u32NuttxAddr=simple_strtoul(getenv(E_MMAP_ID_NUTTX_MEM_ADR),NULL,16);
    //Address Back Up
    stFstrBackUp->u32NuttxAddr=u32NuttxAddr;
    stFstrBackUp->u32AESAddr=u32AesAddr;
    ...
    // TEE BACK UP
    if (TEE_BACKUP(u32NuttxPhyAddr) != 0)
    {
...
U32 MoveNuttx2Dest(U32 NuttxVirtMallocAddr, U32 NuttxSize)
{
    ...
    if(memcmp(pstTeeHd->ucMagicID,TeeMagicID,strlen(TeeMagicID))==0)
    {
        ...
        {
            if (get_value_from_env(E_MMAP_ID_NUTTX_MEM_ADR, NO_DEFAULT_MMAP_VALUE, &pstTeeHd->_sthd.uitee_start_paddr) != 0)
            {
                UBOOT_ERROR("get %s mmap fail\n",E_MMAP_ID_NUTTX_MEM_ADR);
                // when u32NuttxPhyAddr = 0, MoveNuttx2Dest is thought to be failed
                return 0;
            }
        ...
        #endif
#ifdef CONFIG_MSTAR_FAST_STR
        if (STR_BACKUP(u32NuttxPhyAddr) != 0)
        {
            UBOOT_ERROR("STR_BACKUP failed!\n");
            return 0;
        }
                //TEE_BACKUP(u32NuttxPhyAddr);
#endif
    ...
    else
    {
        ...
        if (get_value_from_env(E_MMAP_ID_NUTTX_MEM_ADR, 0xa200000, &u32NuttxPhyAddr) != 0)
        {
            UBOOT_ERROR("get %s mmap fail\n",E_MMAP_ID_NUTTX_MEM_ADR);
            return 0;
        }
        ...
        #else
        memcpy(NuttxPhyMallocAddr, u32NuttxPhyAddr, NuttxSize);
        #endif
    }
...
void _set_SeucreRange(U32 u32NConfigAddr)
{
    ...
    for(i=0;i <(u32MmapGroupCount-1);i++)
    {
        if(pstMmapGroupInfo[i].length!=0)
        {
            ...
            {
                //E_MMAP_ID_VDEC_CPU
                if((pTmpPtr=getenv(E_MMAP_ID_VDEC_CPU_ADR))!= NULL)
                {
                    u32MiuProtectAdr=simple_strtoul(pTmpPtr,NULL,16);
                    if((pTmpPtr=getenv(E_MMAP_ID_VDEC_CPU_LEN))!=NULL)
                    {
                ...
                if((pTmpPtr=getenv(E_MMAP_ID_VDEC_BITSTREAM_ADR))!= NULL)
                {
                    u32MiuProtectAdr=simple_strtoul(pTmpPtr,NULL,16);
                    if((pTmpPtr=getenv(E_MMAP_ID_VDEC_BITSTREAM_LEN))!=NULL)
                    {
                        MIUIndex = (u32MiuProtectAdr >= CONFIG_SYS_MIU_INTERVAL)?1:0;
                        u32MiuProtectLen=simple_strtoul(pTmpPtr,NULL,16);
                        UBOOT_DEBUG("u32MiuProtectAdr  = 0x%x ; miu(%d)\n",u32MiuProtectAdr,MIUIndex);
                ...
                if((pTmpPtr=getenv(E_MMAP_ID_VDEC_SUB_BITSTREAM_ADR))!= NULL)
                {
                    u32MiuProtectAdr=simple_strtoul(pTmpPtr,NULL,16);
                    if((pTmpPtr=getenv(E_MMAP_ID_VDEC_SUB_BITSTREAM_LEN))!=NULL)
                    {
                        MIUIndex = (u32MiuProtectAdr >= CONFIG_SYS_MIU_INTERVAL)?1:0;
                        u32MiuProtectLen=simple_strtoul(pTmpPtr,NULL,16);
                        UBOOT_DEBUG("u32MiuProtectAdr  = 0x%x ; miu(%d)\n",u32MiuProtectAdr,MIUIndex);
...
void SetNuttxAddrToDummyReg(void)
{
    ...
    p_str = getenv ("E_MMAP_ID_NUTTX_MEM_ADR");
    if(NULL != p_str)
    {
        MS_U32 nuttx_start = 0;
        MS_U16 nuttx_start_H = 0;
        UBOOT_INFO("nuttx_start=%s\n",p_str);
        nuttx_start = (MS_U32)simple_strtoul(p_str, NULL, 10);
    ...
    p_str = getenv ("E_MMAP_ID_NUTTX_MEM_LEN");
    if(NULL != p_str)
    {
        MS_U32 nuttx_len = 0;
        MS_U16 nuttx_len_H = 0;
        UBOOT_INFO("nuttx_len=%s\n",p_str);
        nuttx_len = (MS_U32)simple_strtoul(p_str, NULL, 10);
...
{
    U32 u32NuttxPhyAddr = 0x48008000;
    if(argc==2)
       u32NuttxPhyAddr = simple_strtoul(argv[1], NULL, 16);
    UBOOT_DEBUG("[ARM TRUSTZONE Nuttx] boot to 0x%lx\n",u32NuttxPhyAddr);
    *((volatile unsigned int *)(CONFIG_KERNEL_START_ADDRESS + 0x8000 - 0x4)) = u32NuttxPhyAddr; //Nuttx entry address
...
int do_bootNuttx (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    if((u32HwAesPABufAddr==0)||(u32HwAesPABufAddr==0xffffffff)||(u32HwAesBufSize==0)||(u32HwAesBufSize==0xffffffff))
    {
        ...
        if (get_value_from_env(E_MMAP_ID_HW_AES_BUF_LEN, NO_DEFAULT_MMAP_VALUE, &u32HwAesBufSize) != 0)
        {
    ...
    U32 u32NuttxBusAddr = _PA2VA(u32NuttxPhyAddr);

    UBOOT_DEBUG("[ARM TRUSTZONE Nuttx] boot u32NuttxPhyAddr 0x%x\n",u32NuttxPhyAddr);
...
int do_r2_resume(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    UBOOT_TRACE("IN\n");
    U32 u32NuttxStart = 0;
    U32 u32HandShakeAddr=0;
    U32 u32AesAddr=0;
    U32 u32NuttxAddr=0;
    U32 u32TEEStoreSize = TEESTORESIZE;
    U32 u32BootConfigSize = NUTTX_BOOT_CONFIG_SIZE;
    u32HandShakeAddr=simple_strtoul(getenv("E_MMAP_ID_NUTTX_STR_STATUS_ADR"),NULL,16);
    u32NuttxAddr=simple_strtoul(getenv(E_MMAP_ID_NUTTX_MEM_ADR),NULL,16);

    u32HandShakeAddr=MsOS_PA2KSEG1(u32HandShakeAddr);
    //u32HandShakeAddr=u32AesAddr-u32TEEStoreSize-u32BootConfigSize;
    //u32HandShakeAddr=PATCH_STR_HANDSHAKER_ADDR; //need fix this address
    printf("Restart R2 TEE u32HandShakeAddr =0x%x\n",u32HandShakeAddr);
    *((U32*)u32HandShakeAddr)= 0x5A5A5A5A;
    UBOOT_DEBUG("Restart R2 TEE\n");
    MApi_R2_StartFrDRAM(u32NuttxAddr);
    UBOOT_DEBUG("Wait forTEE Resume done\n");
    if(-1==IsTEE_Resume_Done(u32HandShakeAddr))
    {

...
_SUB_SECURE_INFO* getSubSecureInfo(char* partition,BOOLEAN bIsInterleaveMode)
{
    ...
    if((GetSecureMergeType() == E_SECUREMERGE_NONE) || (IsFileSystemPartition(partition) == 0))
    {
        ...
        else if(0 == strcmp("RFS",partition))
        {
            if(TRUE == bIsInterleaveMode)
            {
                pSubAddr = &(stSecureInfo.data_interleave.RootFs[0]);
            }
            else
            {
                pSubAddr = &(stSecureInfo.data.RootFs[0]);
            }
        }
        ...
        else if(0 == strcmp("CON",partition))
        {
            if(TRUE == bIsInterleaveMode)
            {
...
int do_Verify_System(char* partition, int CacheBufAddr)
{
    ...
    UBOOT_TRACE("IN\n");
    ret = GetImageSize(partition, 0, &image_size);
    if (0 != ret)
    {
    ...
    if(GetSecureMergeType() == E_SECUREMERGE_NONE)
    {
        ...
        UBOOT_DEBUG("For Verify_System function, the secure_info should be merged into %s\n", partition);
        return -1;
    }
    ...
    flush_cache((U32)VirtWorkBuf, (U32)image_size);
    GetPublicKeyN(E_RSA_UPGRADE_PUBLIC_KEY, u8ImagePublicKeyN, sizeof(u8ImagePublicKeyN));
    GetPublicKeyE(E_RSA_UPGRADE_PUBLIC_KEY, u8ImagePublicKeyE, sizeof(u8ImagePublicKeyE));
    GetSignature(partition, 0, u8ImageSignature, sizeof(u8ImageSignature));
    if (-1 == Secure_doAuthentication(u8ImagePublicKeyN, u8ImagePublicKeyE, u8ImageSignature, VirtWorkBuf, image_size))
    {
...
#if (defined(CONFIG_AUTH_KEYBANK_RSA_KEY_FROM_HASH2) && (ENABLE_MSTAR_NS_UBOOT == 0))
...
int LoadCustomerKeyBank(void)
{
    ...
    u32HwAesVABufAddr=PA2VA(u32HwAesPABufAddr);
...
int do_ReadKL(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    #if (CONFIG_VERIFY_SYSTEM == 1)
    if(do_Verify_System(SYSTEM, (int)BOOT_MODE_TMP_BUF_ADDR) != 0)
    {
    ...
    //Authenticate Kernel
    if(do_Authenticate_RawData(KERNEL,(int)BOOT_MODE_TMP_BUF_ADDR)!=0)
    {
...
int do_BootKL(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    //Boot Kernel
    snprintf(buffer, CMD_BUF, "bootm 0x%x",(int)BOOT_MODE_TMP_BUF_ADDR);

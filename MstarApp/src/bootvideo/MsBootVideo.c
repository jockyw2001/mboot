...
static void _SaveBootVideoBootArg(MsDrv_VIDEO_CodecType_e eVdecCodecType)
{
    ...
    MS_U32 u32FrameCntAddr = 0;
    MS_U32 u32SkipFrameCntAddr = 0;


    u32PtsAddr = MsDrv_VideoGetPtsAddr(eVdecCodecType, &u32PtsKhzUnit);

    sprintf(szText, ARG_BOOT_VIDEO_PTS_ADDR"=0x%lx", u32PtsAddr);
    ...
    if (bRet == TRUE)
    {
        sprintf(szText, ARG_BOOT_VIDEO_FRAME_CNT"=%ld", u32FrameCntAddr);
        set_bootargs_cfg(ARG_BOOT_VIDEO_FRAME_CNT, szText, 1); // don't need to save

        sprintf(szText, ARG_BOOT_VIDEO_SKIP_FRAME_CNT"=%ld", u32SkipFrameCntAddr);
        set_bootargs_cfg(ARG_BOOT_VIDEO_SKIP_FRAME_CNT, szText, 1); // don't need to save
    }
}

static unsigned int Load_Video2Dram(char *SrcPath,U32 Dram_Addr)
{
    char PathBuf[128] = "\0";
...
int do_bootvideo(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    {
        char PathBuf[128] = "\0";
        if (get_addr_from_mmap("E_MMAP_ID_VIDEO_FILE_IN", (U32 *) &u32FileinAddr) !=0)
        {
        ...
        u32FileInSize = Load_Video2Dram(PathBuf, u32FileinAddr);
        u32AutoScan = 0;
    }
    ...
    else
    {
        ...
        if (MsDrv_DmxFileInInit(u32FileinAddr, u32FileInSize, u32Repeat) != 0)
        {
    ...
    snprintf(szText, MAX_BOOT_VIDEO_TEXT_LENGTH, "%s=0x%lx",ENV_BOOT_VIDEO_FILE_ADDR, u32FileinAddr);

...
#ifdef __AEONR2__
#if (ENABLE_MSTAR_MARIA10 || ENABLE_MSTAR_MACAW12 || ENABLE_MSTAR_EDEN || ENABLE_MSTAR_EULER || ENABLE_MSTAR_MAYA || ENABLE_MSTAR_NASA || ENABLE_MSTAR_WHISKY)
...
//========================================================
#if ( defined(__mips__) && ENABLE_MSTAR_MILAN ) // Non-OS - Milan
    ...
    if (bIsUBoot == TRUE) //The following binary is U-Boot.
    {
        ...
        else //It implys that U-Boot is not compressed.
        {
          ...
          "1:\n"
            ...
            // Uncompressed format
#elif ( (ENABLE_MSTAR_KRONUS==1) || (ENABLE_MSTAR_KAISERIN==1) || (ENABLE_MSTAR_KENYA == 1)|| (ENABLE_MSTAR_KRITI == 1) || (ENABLE_MSTAR_KRATOS))
            asm volatile (
            // Load ROM_START, RAM_START, RAM_END, ROM_END from 0xBFC20000
            #ifdef CONFIG_MSTAR_KAISERIN
                "li      $15, 0x94020000;"
...
LOAD_CODE_POINT:
    ...
    Aeon_printf("IsCompressMagicNumber=0x%X\n", IsCompressMagicNumber);
    ...
    if ( IsCompressMagicNumber )
    {
        ...
        if ( u32CRC32_HW != u32CRC32_SW )
        {
            Aeon_printf("CRC error: u32CRC32_HW=0x%X,", u32CRC32_HW);
            Aeon_printf(" u32CRC32_SW=0x%X\n", u32CRC32_SW);

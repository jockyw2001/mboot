...
for((i=1;i<=${#array_fe_type[*]} ;i++));do
    ...
    for((j=1;j<=${#array_ddr[*]} ;j++));do
        dircation=./configs/HB_keres/"${array_ddr[j]}""MHZ"/
        ...
        do
            ...
            make
            source_bin_path=./bin
            target_bin_path=./bin/keres/"${array_fe_type[i]}"/HF/Normal/"${array_ddr[j]}""MHZ"
...
target_HB_path=../../../Hummingbird/MainTrunk/DVBT/boot/mboot/bin/keres/ 
if [ -d "$target_HB_path" ]; then
        cp -r bin/keres/* $target_HB_path

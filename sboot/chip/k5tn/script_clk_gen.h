...
WREG(REG_ADDR_BASE + (0x100b02 << 1), 0x001d), //clk_bist  
                                               //Bit[0]: gate clk_bist 
                                               //Bit[4:2]: SRAM bist test clk 7:xtal
...
WREG(REG_ADDR_BASE + (0x100b3c << 1), 0x0100), // clk_mpll_syn = 432M (for VEPLL  0: 216M, 1:432M)
                                               // clk_mpll_syn2 = 216MHz
WREG(REG_ADDR_BASE + (0x100bf8 << 1), 0x0100), // clk_fsync = 432M
#if(defined(CONFIG_MSD7X321C) && (CONFIG_MSD7X321C==1))
...
WREG(REG_ADDR_BASE + (0x100b52 << 1), 0x0400), // clk_ts2 = CLK_TS0_IN
                                               // clk_ts3 = CLK_TS1_IN                                             
// K5TN update (no NDS)  
WREG(REG_ADDR_BASE + (0x100b54 << 1), 0x0004), // clk_tsp = 172MHz
                                               // [4:2] 000=172MHz (reg_ckg_tsp)
// VD_MHEG5 K5TN update
WREG(REG_ADDR_BASE + (0x100b60 << 1), 0x001c), // ckg_vd_mheg5 = 240MHz

// R2
WREG(REG_ADDR_BASE + (0x100b62 << 1), 0x0034), //clk_r2 = 288MHz

// HVD
WREG(REG_ADDR_BASE + (0x100b68 << 1), 0x0004), // CLK_HVD=192MHz

// JPD
WREG(REG_ADDR_BASE + (0x100b6a << 1), 0x000c), // ckg_jpg = 192MHz (0:72m 4:108m 8:144m c:192m)
...
// K5TN update
#if(defined(CONFIG_MSD7X321C) && (CONFIG_MSD7X321C==1))
WREG(REG_ADDR_BASE + (0x100b72 << 1), 0x0010), // clk_mvd = 86MHz
                                               // clk_parser = 172Mhz
#else
WREG(REG_ADDR_BASE + (0x100b72 << 1), 0x0000), // clk_mvd = 216MHz
                                               // clk_parser = 172Mhz
#endif
...
// GPD  K5TN update
WREG(REG_ADDR_BASE + (0x100b7c << 1), 0x0000), // clk_gpd_p = 216MHz
...
WREG(REG_ADDR_BASE + (0x100b86 << 1), 0x0000), // clk_psram0 on
                                               // clk_psram1 on
// GE
WREG(REG_ADDR_BASE + (0x100b90 << 1), 0x0000), // clk_ge = clk_miu 266MHz
// STB_DC
WREG(REG_ADDR_BASE + (0x100b98 << 1), 0x0010), // clk_dc0 = 123M

// SC
WREG(REG_ADDR_BASE + (0x100ba2 << 1), 0x0400), // clk_ficlk = 192M (clk_fclk)
WREG(REG_ADDR_BASE + (0x100ba4 << 1), 0x1000), // clk_fclk = 192MHz
...
WREG(REG_ADDR_BASE + (0x100bb0 << 1), 0x000c), // clk_hdgen = 27M (clk_odlck)
...
WREG(REG_ADDR_BASE + (0x100b5e << 1), 0x0000), // clk_stamp=27MHz

// Gmac
WREG(REG_ADDR_BASE + (0x103364 << 1), 0x0010), // clk_secemac_ahb = 172MHz

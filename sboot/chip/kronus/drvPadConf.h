...
#define _MapBase_nonPM_K1 (RIU_MAP + 0x200000)
#define _MapBase_PM_K1    RIU_MAP
#else
#define _MapBase_nonPM_K1 0xA0200000
#define _MapBase_PM_K1   0xA0000000
#endif
...
//---------------------------------------------------------------------
#if (MS_BOARD_TYPE_SEL != BD_MST124E_D01A_S && MS_BOARD_TYPE_SEL != BD_MST124E_D01A_SZ && MS_BOARD_TYPE_SEL != BD_MST140A_D01A_S && MS_BOARD_TYPE_SEL != BD_MST140A_D01A_S && MS_BOARD_TYPE_SEL != BD_MST124G_D01A_S && MS_BOARD_TYPE_SEL != BD_MST124G_D01A)
    _RVM1(0x1E45, 0, BIT2), // reg_156_mode set to 0 if the package is not 156 pin for SMC
#else
    _RVM1(0x1E45, 4, BIT2), // reg_156_mode set to 1 if the package is not 156 pin for SMC
#endif
//---------------------------------------------------------------------
    // UART_SRC_SEL
...
#if ((MS_BOARD_TYPE_SEL==BD_MST124F_D01A_S) || (MS_BOARD_TYPE_SEL==BD_MST124F_SZ_Demo)|| (MS_BOARD_TYPE_SEL==BD_MST124F_D02A))
    #if (PAD_HDMI_SDA_IS_GPIO != GPIO_NONE)
     _MEMMAP_ADDR(0x11),  // high bank
     _RVM1(0x3005, 0, BIT7),
    #if (PAD_HDMI_SDA_IS_GPIO != GPIO_IN)
    #if (PAD_HDMI_SDA_IS_GPIO == GPIO_OUT_HIGH)
     _RVM1(0x3002, BIT4, BIT4),
    #else
     _RVM1(0x3002, 0, BIT4),
    #endif
    #endif
...
#endif
     _MEMMAP_nonPM_,
#if ((MS_BOARD_TYPE_SEL == BD_MST140A_D01A_S) || (MS_BOARD_TYPE_SEL == BD_MST140A_D01A))
    // reg_chiptop_dummy_3_0[0] : reg_kgb2_mode for KGB supporting smart card
    _RVM1(0x1E46, BIT2, BIT2),
    _END_OF_TBL2_,
#endif
...
//---------------------------------------------------------------------
    ...
    _END_OF_TBL2_,
        ...
        (MS_BOARD_TYPE_SEL==BD_MST124A_D01A)||\
        (MS_BOARD_TYPE_SEL==BD_MST124B_D01A_S)||\
        (MS_BOARD_TYPE_SEL==BD_MST124A_D02A_S)||\
        (MS_BOARD_TYPE_SEL==BD_MST140A_D01A_S)||\
        (MS_BOARD_TYPE_SEL==BD_MST140A_D01A))
    //external flash #1
    _MEMMAP_PM_,
    _RVM1(0x0E6A, 0x00, BIT0), //reg_spi_gpio set to 0 (swtch to spi_do, spi_di, spi_ck)
    _RVM1(0x0E6A, 0x00, BIT1), //reg_spicsz0_gpio set to 0 (enable spi_cz0)

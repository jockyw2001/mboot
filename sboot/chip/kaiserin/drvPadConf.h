...
#define _MapBase_nonPM_K2 (RIU_MAP + 0x200000)
#define _MapBase_PM_K2    RIU_MAP
#else
#define _MapBase_nonPM_K2 0xA0200000
#define _MapBase_PM_K2   0xA0000000
#endif
...
//---------------------------------------------------------------------
...
#if (MS_BOARD_TYPE_SEL == BD_MST138B_D01D)
	// this cypher is to activate GPIO_PM4, only in 138B-D01D
    ...
    _RVM1(0x0E6A, BIT2, BIT2), //reg_spicsz1_gpio set to 1
    ...
    #if(PAD_LED_PM0_IS_GPIO != GPIO_NONE)
    #define PAD_LED_PM0_OEN (PAD_LED_PM0_IS_GPIO == GPIO_IN ? BIT1: 0)
    #define PAD_LED_PM0_OUT (PAD_LED_PM0_IS_GPIO == GPIO_OUT_HIGH ? BIT1: 0)
    _RVM1(0x0ecd, BIT1, BIT1),
    _RVM1(0x0ecf, PAD_LED_PM0_OUT, BIT1),
    _RVM1(0x0ed3, PAD_LED_PM0_OEN, BIT1),
    #endif

    #if(PAD_LED_PM1_IS_GPIO != GPIO_NONE)
    #define PAD_LED_PM1_OEN (PAD_LED_PM1_IS_GPIO == GPIO_IN ? BIT2: 0)
    #define PAD_LED_PM1_OUT (PAD_LED_PM1_IS_GPIO == GPIO_OUT_HIGH ? BIT2: 0)
    _RVM1(0x0ecd, BIT2, BIT2),
    _RVM1(0x0ecf, PAD_LED_PM1_OUT, BIT2),
    _RVM1(0x0ed3, PAD_LED_PM1_OEN, BIT2),
    #endif
...
//---------------------------------------------------------------------
    ...
    #define TS0_MODE ((PADS_TS0_MODE == 1) ? (BIT0) : \
                      ...
                      (PADS_TS0_MODE == 3) ? (BIT1|BIT0) : \
                      (PADS_TS0_MODE == 4) ? (BIT2) : \
                      (PADS_TS0_MODE == 5) ? (BIT2|BIT0) : \
                      (PADS_TS0_MODE == 6) ? (BIT2|BIT1) : \
                      (PADS_TS0_MODE == 7) ? (BIT2|BIT1|BIT0) : \
                      (PADS_TS0_MODE == 8) ? (BIT3) : \
                      (PADS_TS0_MODE == 9) ? (BIT3|BIT0) : 0)
    ...
    #define I2S_OUT_MODE ((PADS_I2S_OUT_MODE == PAD_CI_D2  ) ? (BIT5) : \
                          (PADS_I2S_OUT_MODE == PAD_I2S_OUT_BCK) ? (BIT6) : \
                          (PADS_I2S_OUT_MODE == PAD_TS1_D5) ? (BIT6|BIT5) : 0)
...
#if (MS_BOARD_TYPE_SEL == BD_MST138B_D01A_S) || (MS_BOARD_TYPE_SEL == BD_MST138B_D01A) || (MS_BOARD_TYPE_SEL == BD_MST138B_D01D) || (MS_BOARD_TYPE_SEL == BD_MST178A_D01A_S) || (MS_BOARD_TYPE_SEL == BD_MST178A_D01A)
...
//---------------------------------------------------------------------
#if (MS_BOARD_TYPE_SEL == BD_MST138B_D01A) || (MS_BOARD_TYPE_SEL == BD_MST138B_D01D)
    //external flash #1
    _MEMMAP_PM_,

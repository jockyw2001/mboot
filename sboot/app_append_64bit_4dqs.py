...
def gen_bin(boot_from, flash, header, app_zip, dst, entry, zip_size, size, HEADER_SIZE, app_bin_path):
    if boot_from == 'bfe':
        ...
        hash0_bin    = flash[_ld_HASH0_run_start - _ld_load_start:_ld_HASH1_run_start - _ld_load_start]
        hash1_table  = flash[_ld_HASH1_run_start - _ld_load_start:_ld_HASH1_run_end - _ld_load_start]
        hash1_code   = open(join(bindir, HASH1_CODE), 'rb').read()
        miu_bin  = hash1_table + hash1_code + '\0' * (HASH1_SIZE-len(hash1_table)-len(hash1_code))
        loader_bin   = flash[_ld_LDR_load_start - _ld_load_start:_ld_LDR_load_end  - _ld_load_start]
        bfe_boot_bin =  hash0_bin + miu_bin + loader_bin
        ...
        hash0_bin      = flash[_ld_HASH0_run_start - _ld_load_start:_ld_HASH1_run_start - _ld_load_start]
        hash1_table    = flash[_ld_HASH1_run_start - _ld_load_start:_ld_HASH1_run_end - _ld_load_start]
        hash1_table    = hash1_table +  '\0' * ((4-len(hash1_table)%4)%4) #4 byte alignment
        hash1_code     = open(join(bindir, HASH1_CODE), 'rb').read()
        bfn_header     = '\0' * 0x100
        miu_bin        = bfn_header + hash0_bin + hash1_table + hash1_code
        bfn_header     = pack('<4cII','P','A','R','M', 0 , len(miu_bin)) + bfn_header[12:] #0x100 header: TAG"PARM" + reserved(4) + size(3) + reserved(left)
        miu_bin        = bfn_header + miu_bin[0X100:]
        miu_bin        = miu_bin + '\0' * (HASH1_SIZE-len(hash1_table)-len(hash1_code)) + 'PARE'
        ...
        flash = miu_bin + '\0' * (0x7000-len(miu_bin)) + flash[0x7000:]
        flash = flash[:0x7000] + 'LDR0' + flash[_ld_LDR_load_start - _ld_load_start+4:] #tag of hash2 for bfn case
...
def main(argv):
    ...
    global _ld_load_start, AppDigest, _ld_LDR_run_base, _ld_REE_LDR_run_base, _ld_REE_LDR_load_start, bindir, HASH1_CODE
...
# End extract TEE_loader.bin
    ...
    #for generate bfs_boot.bin
    hash1_table    = flash[_ld_HASH1_run_start - _ld_load_start:_ld_HASH1_run_end - _ld_load_start]
    hash1_table    = hash1_table +  '\0' * ((4-len(hash1_table)%4)%4) #4 byte alignment
    hash1_code     = open(join(bindir, HASH1_CODE), 'rb').read()
    miu_bin        = hash1_table + hash1_code
    miu_bin        = miu_bin + '\0' * (HASH1_SIZE-len(hash1_table)-len(hash1_code))
    bfs_bin        = flash[:0x1000] + miu_bin + flash[0x3000:len(flash)]
    flash          = flash[:0x1000] + miu_bin + flash[0x3000:len(flash)]

    if os.path.exists(join(bindir,BFS_BOOT_OUT)):
        os.remove(join(bindir,BFS_BOOT_OUT))
    open(join(bindir,BFS_BOOT_OUT), 'wb').write(bfs_bin)
    print PREFIX, 'Generate bin/bfs_boot.bin'

...
void HAL_CIPHER_DMA_AlgoTable_Init(void)
{
    ...
    _memset(_CryptodMA_AlgoTable , 0x0 , sizeof(_CryptodMA_AlgoTable));
...
MS_BOOL HAL_CIPHER_DMA_Set_OutputDstTo(CIPHER_MEM_TYPE OutputDstTo, MS_U8* pu8Data, MS_U32 u32Size)
{
	...
	switch(OutputDstTo)
	{
		...
		case E_CIPHER_KL_M:
		case E_CIPHER_KL_BC:
            HAL_DEBUG_CIPHER(CIPHER_DBG_ERR, printf("[%s][%d]Not support output destination to Keyladder\n", __func__, __LINE__));
            return FALSE;
...
void HAL_CIPHER_PARSER_Set_Mode(CIPHER_PARSER_MODE eMode)
{
    ...
    switch(eMode)
    {
        ...
        case E_CIPHER_PARSER_HDCP20_PKT188_CLEAR:
            ...
            break;
        default:
            HAL_DEBUG_CIPHER(CIPHER_DBG_ERR, printf("Error: Not suport parser mode!\n"));
            break;
    }
...
MS_BOOL HAL_CIPHER_PARSER_Set_Key2(DRV_CIPHER_KEY stKey)
{
    ...
    if(E_DMA_ALGO_NONE == algo)
    {
        ...
        bDK2 = FALSE;
        bTwoKey = FALSE;
        _memset(_pu8CipherKey2, 0x0 , HAL_CRYPTODMA_KEYLEN_MAX);
        goto SET_KEY_DONE;
    }
...
MS_BOOL HAL_CIPHER_PARSER_Set_IV2(MS_U8 *pu8IV2, MS_U8 u8IVLen)
{
    ...
    {
        _memset(_pu8IV2, 0, CIPHER_KEY_SIZE);

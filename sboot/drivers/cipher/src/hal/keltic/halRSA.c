...
static REG_RSACtrl          *_RSACtrl = (REG_RSACtrl*)REG_RSACTRL_BASE;


//--------------------------------------------------------------------------------------------------
//  Macro of bit operations
//--------------------------------------------------------------------------------------------------
void _RSA_REG32_W(REG32 *reg, MS_U32 u32Value)
{
     (*((volatile MS_U32*)(reg))) = u32Value;
}

MS_U32 _RSA_REG32_R(REG32 *reg)
{
    MS_U32 u32Value;
    u32Value = (*(volatile MS_U32*)(reg));
    return u32Value;
}
...
//  Inline Function
//--------------------------------------------------------------------------------------------------
void HAL_RSA_SetBank(MS_U32 u32NonPmBankAddr)
{
...
void HAL_RSA_Ind32Ctrl(MS_U8 u8Direction)
{
    //[1] reg_ind32_direction 0: Read. 1: Write

    if(u8Direction==RAM_WRITE)
    {
...
void HAL_RSA_SetKeyLength(MS_U32 u32KeyLen) /// KeyLen  Unit: Word
{
...
void HAL_RSA_SetKeyType(MS_U8 u8HwKey, MS_U8 u8PubKey)
{
    ...
    MS_U32 u32KeyType = (_u32KeyLen<<8) & RSA_CTRL_KEY_LENGTH_MASK;

    if(u8HwKey==1)
    {
        u32KeyType |= RSA_CTRL_SEL_HW_KEY;
    }
    else
    {
        u32KeyType &= ~RSA_CTRL_SEL_HW_KEY;
    ...
    if(u8PubKey==1)
    {
        u32KeyType |= (RSA_CTRL_SEL_PUBLIC_KEY);
    }
    else
    {
        u32KeyType &= ~RSA_CTRL_SEL_PUBLIC_KEY;
    ...
    _RSA_REG32_W((&_RSACtrl[0].Rsa_Ctrl), u32KeyType);
}
...
void HAL_RSA_Rootkey_Sel(MS_U8 u8KeySrc)
{
    ...
    switch(u8KeySrc)
    {
        ...
        {
            u32KeySel &= ~RSA_ROOTKEY_SEL_TYPE;
            break;
        }
        case E_RSA_HW_PUB_KEY1:  // 10
        {
            u32KeySel |= RSA_ROOTKEY_SEL_TYPE;
            u32KeySel &= ~RSA_ROOTKEY_SEL_PUB_NO;
            break;
        }
    ...
    _RSA_REG32_W((&_RSACtrl[0].Rsa_Ind32_Start), u32KeySel);
}

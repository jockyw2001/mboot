...
unsigned int cfg_env_bak_offset = 0;
//MSTAR end
...
int saveenv(void)
{
	...
	if(ubi_find_volume(ENV_VOL_NAME))
	{
		...
		ret = writeenv(cfg_env_bak_offset, (u_char *) &env_new);
		if (ret)
		{
			puts("Backup Failed\n");
			return ret;
...
void env_relocate_spec (void)
{
	...
	if(cfg_env_offset == 0)
	{
		...
		if(CONFIG_ENV_SIZE <= ubi_leb_sz)
		{   
		    cfg_env_offset = size - (cfg_env_offset*ubi_leb_sz);
			cfg_env_bak_offset = cfg_env_offset + ubi_leb_sz;
		}
		else
		{
			cfg_env_offset = size - (cfg_env_offset*ubi_leb_sz*2);
		    cfg_env_bak_offset = cfg_env_offset + ubi_leb_sz * 2;
		}
	}
	//find ENV volume
